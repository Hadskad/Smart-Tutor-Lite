...
export async function generateStudyNotesWithMeta(
  ...
  const systemPrompt = `You are an expert educational content processor specialized in converting lecture transcripts into exceptional study notes.
...
${STUDY_NOTES_JSON_SCHEMA_PROMPT}`;
  const userPrompt = `Transform the following lecture transcript into beautifully formatted, comprehensive study notes.
